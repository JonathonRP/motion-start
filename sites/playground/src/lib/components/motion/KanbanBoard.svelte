...
<script lang="ts">
	...
	let cards = $state<Card[]>([
		{ id: 'fix-bifrost-ci-pipeline', title: 'Fix Bifrost CI pipeline', column: 'todo', order: 0 },
		{ id: 'add-dr-strange-to-on-call', title: 'Add Dr. Strange to on-call', column: 'todo', order: 1 },
		{ id: 'order-pym-particles-bulk', title: 'Order Pym Particles (bulk)', column: 'todo', order: 2 },
		{ id: 'take-over-universe-backlog', title: 'Take over universe (backlog)', column: 'todo', order: 3 },
		{ id: 'stop-thanos-deprioritised', title: 'Stop Thanos (deprioritised)', column: 'inprogress', order: 0 },
		{ id: 'refactor-vibranium-service', title: 'Refactor Vibranium service', column: 'inprogress', order: 1 },
		{ id: 'recruit-loki-he-quit', title: 'Recruit Loki (he quit)', column: 'done', order: 0 },
		{ id: 'i-am-inevitable', title: 'I am inevitable ✅', column: 'done', order: 1 },
	]);

	let preview = $state<PreviewState>({ col: null, index: null });
	...
	function hasCrossColumnPreview() {
		return Boolean(
			draggingCardId && draggingFromCol && preview.col && preview.col !== draggingFromCol && preview.index !== null
		);
	}
	...
	function getRenderOrder(card: Card) {
		...
		if (card.id === draggingCardId) return preview.index!;
		if (card.column === preview.col && card.order >= preview.index!) return card.order + 1;
		return card.order;
	}
	...
	function getColumnSlots(colId: ColumnId) {
		return getCommittedColumnCards(colId)
			...
			.map((card) => {
				const element = document.getElementById(`kanban-task-${card.id}`);
				return element ? { card, rect: element.getBoundingClientRect() } : null;
			})
			.filter((slot): slot is { card: Card; rect: DOMRect } => slot !== null)
	...
	function moveCardToColumn(cardId: string, currentCol: ColumnId, targetCol: ColumnId, targetIndex: number) {
		...
		const sourceCards = getCommittedColumnCards(currentCol).filter((card) => card.id !== cardId);
	...
	function handleDrag(card: Card, info: PanInfo) {
		...
		for (const colId of COLUMN_IDS) {
			...
			const colRect = ref.getBoundingClientRect();
			if (px <= colRect.left || px >= colRect.right || py <= colRect.top || py >= colRect.bottom) {
				continue;
			}
			...
			setPreview({ col: colId, index: idx });
	...
	function handleDrop(cardId: string) {
		if (draggingFromCol && preview.col !== null && preview.index !== null && preview.col !== draggingFromCol) {
			moveCardToColumn(cardId, draggingFromCol, preview.col, preview.index);
		}
						...
						{#snippet children({ item: card })}
							{#if rendersInColumn(card, colId)}
								<Reorder.Item
									...
									transition={{
										rotate: { type: 'spring', damping: 12, stiffness: 200 },
