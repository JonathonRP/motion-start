<script lang="ts">
	...
	const position = $derived(
		page.url.searchParams.get("position") || "static",
	);

	const itemStyle = $derived.by<MotionStyle>(() =>
		position === "relative"
			? {
					width: "100px",
					height: "100px",
	...
	function fadePurple(node: HTMLDivElement) {
		node.animate([{ opacity: 0 }, { opacity: 1 }], { duration: 1000 });
		node.animate([{ opacity: 1 }, { opacity: 0.5 }], { duration: 1000 });
	}
</script>
...
>
	...
	<div
		{@attach fadePurple}
		style="width: 100px; height: 100px; background-color: purple;{position ===
		'relative'
