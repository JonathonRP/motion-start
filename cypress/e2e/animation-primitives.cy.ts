// Core Animation Primitives - Tests basic animation types (spring, tween, keyframes)
// Coverage: /tests route with spring, repeat, reverse, hover, tap, drag effects
describe('Animation Primitives', () => {
	it('renders and animates core animation types', () => {
		cy.visit('/tests');
		...
		cy.get('#scrollProg').then(($el) => {
			const initTransform = $el.css('transform');
			cy.get($el)
				.realMouseDown()
				.wait(550)
				.realSwipe('toBottom', { length: 500 })
				.should((elAf) => {
					const newTransform = elAf.css('transform');
					expect(initTransform).to.not.equal(newTransform);
				});
		...
		cy.get('#presswipe').then(($el) => {
			const inittext = $el.text();
			cy.get($el)
				.realSwipe('toLeft', { length: 200 })
				.realSwipe('toRight', { length: 200 })
				.realSwipe('toLeft', { length: 200 })
				.realSwipe('toRight', { length: 200 })
				.realSwipe('toLeft', { length: 200 })
				.should((elAf) => {
					const newText = elAf.text();
					expect(inittext).to.not.equal(newText);
				});
		});
			...
			cy.get($el)
				...
				.then((el) => {
					...
					cy.get(el)
						...
						.should((elAf) => {
							const newTransform = elAf.css('transform');
							expect(initialTransform).to.not.equal(newTransform);
						});
				});
		});

		cy.get('#reorder').then(($el) => {
			const inittext = $el.css('transform');
			cy.get($el)
				.realSwipe('toLeft', { length: 200 })
				.should((elAf) => {
					const newText = elAf.css('transform');
					expect(inittext).to.not.equal(newText);
				});
		});
	});
