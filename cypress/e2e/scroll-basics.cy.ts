...
// Tests whileInView, scroll progress tracking, and scroll-linked animations
// Coverage: Viewport intersection animations, scroll position tracking, transform based on scroll
describe('Scroll-Triggered Animations', () => {
	it('animates element when entering viewport with whileInView', () => {
		...
		cy.get('#while-in-view-box', { timeout: 10000 })
			.should('exist')
			.should(($el) => {
	...
	it('animates based on scroll position', () => {
		...
		cy.get('#scroll-animated-box', { timeout: 10000 })
			.should('exist')
			.should(($el) => {
				expectBox($el[0].getBoundingClientRect()).to.be.visible();
			});
