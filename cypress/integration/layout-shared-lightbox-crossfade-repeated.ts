const pipe =
	...
	(value: T) =>
		fns.reduce((latest, fn) => fn(latest), value);
...
describe.skip('Shared layout lightbox example, toggle back and forth', () => {
	it('Correctly animates back and forth with crossfade', () => {
		function open(session: Cypress.cy) {
			return session
				...
				.wait(50)
				.should(([$box]: any) => {
					expect(window.getComputedStyle($box).borderRadius).to.equal('2.25225% / 0.88968%');
				...
				.should(([$box]: any) => {
					expect(window.getComputedStyle($box).borderRadius).to.equal('2.25225% / 0.88968%');
					expect(window.getComputedStyle($box).opacity).to.equal('0.6');
					expectBbox($box, {
						top: 49,
						left: 209,
						width: 222,
		...
		function close(session: Cypress.cy) {
			return session
				...
				.get('#item-parent')
				.should(([$box]: any) => {
					expect(window.getComputedStyle($box).borderRadius).to.equal('0.271444% / 0.0864753%');
					expect(window.getComputedStyle($box).opacity).to.equal('1');
					expectBbox($box, {
				...
				.should(([$box]: any) => {
					expect(window.getComputedStyle($box).borderRadius).to.equal('0.271444% / 0.0864753%');
					expect(window.getComputedStyle($box).opacity).to.equal('0.0666667');
					expectBbox($box, {
						height: 578.199951171875,
						left: 209.89999389648438,
						top: 40.900001525878906,
		...
		pipe(
			...
			open,
			close
		)(cy.visit(`?test=layout-shared-lightbox-crossfade&type=crossfade&partial-ease=true`).wait(50));
	});
	it.skip('Correctly animates back and forth with switch', () => {
		...
		pipe(
			...
			open,
			close
		)(cy.visit(`?test=layout-shared-lightbox-crossfade&type=switch&partial-ease=true`).wait(50));
	});
});
