describe('Tabs demo', () => {
	afterEach(() => {
		...
		cy.get('body').trigger('pointercancel', { force: true });
	});

	it('New items correctly reorderable', () => {
		cy.visit('?test=drag-tabs&case=new-item')
			.get('button.add-item')
			.wait(50)
			.click()
			.wait(150)
			...
			.wait(30)
			.trigger('pointermove', -30, 10, { force: true }) // Gesture will start from first move past threshold
			.wait(100)
			.trigger('pointerup', { force: true })
			.wait(250)
			.then(([$label]: any) => {
				const { left } = $label.getBoundingClientRect();
				expect(left).to.be.lessThan(580);
			});
	});
	...
	it("Double removing item doesn't break exit animation", () => {
		...
		cy.visit('?test=drag-tabs')
			...
			.then(([$button]: any) => {
				...
				removeButton.dispatchEvent(new PointerEvent('pointerup', { bubbles: true }));
			})
			.wait(20)
			.then(() => {
				removeButton.dispatchEvent(new PointerEvent('pointerdown', { bubbles: true }));
