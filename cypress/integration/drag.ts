...
describe('Drag', () => {
	...
	it('rescales draggable element in relation to resized constraints', () => {
		cy.visit('?test=drag-ref-constraints-resize')
			...
			.should(([$constraints]: any) => {
				const { left, top, right, bottom } = $constraints.getBoundingClientRect();
	...
	it('Snaps to cursor', () => {
		cy.visit('?test=drag-snap-to-cursor').wait(200);
		cy.scrollTo(0, 800)
			.get('#scrollable')
			.should(([$scrollable]: any) => {
				const { top, left, right, bottom } = $scrollable.getBoundingClientRect();
				expect(top).to.equal(200);
