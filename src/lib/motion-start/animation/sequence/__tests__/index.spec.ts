...
describe('createAnimationsFromSequence', () => {
	...
	test('It creates a single animation', () => {
		const animations = createAnimationsFromSequence(
			[[a, { opacity: 1 }, { duration: 1, ease: [0, 1, 2, 3], times: [0, 1] }]],
			undefined,
			undefined,
			{ spring }
		);
	...
	test('It creates a single animation with defaults', () => {
		const animations = createAnimationsFromSequence([[a, { opacity: 1 }, { duration: 1 }]], undefined, undefined, {
			spring,
		});
	...
	test('It sequences one animation after another', () => {
		const animations = createAnimationsFromSequence(
			[
				[a, { x: [100, 200, 300], opacity: 1 }, { duration: 0.5, ease: 'easeInOut' }],
		...
		expect(animations.get(a)!.keyframes.x).toEqual([100, 200, 300, null, 400]);
		expect(animations.get(a)!.transition.x).toEqual({
			duration: 2,
	...
	test('It accepts motion values', () => {
		const animations = createAnimationsFromSequence([[value, 100, { duration: 0.5 }]], undefined, undefined, {
			spring,
		});
	...
	test('It accepts motion values keyframes', () => {
		const animations = createAnimationsFromSequence([[value, [50, 100], { duration: 0.5 }]], undefined, undefined, {
			spring,
		});
	...
	test('It scales the whole animation based on the provided duration', () => {
		const animations = createAnimationsFromSequence(
			[[a, { opacity: 1 }, { duration: 1, ease: ['easeOut'], times: [0, 1] }]],
			{ duration: 2 }
		);
	...
	test('It passes default options to children', () => {
		const animations = createAnimationsFromSequence([[a, { opacity: 1 }, { times: [0, 1] }]], {
	...
	test('It correctly adds type: spring to timeline with simulated spring', () => {
		const animations = createAnimationsFromSequence(
			[
				[a, { x: 200 }, { duration: 1 }],
				[a, { x: [100, 0] }, { type: 'spring', stiffness: 200, damping: 10 }],
			],
			undefined,
			undefined,
			{ spring }
