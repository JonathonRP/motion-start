 ...
 * (requestAnimationFrame is not available in Bun test environment)
 */
...
function testAnimate<V extends string | number>(
	...
	new MainThreadAnimation({
		...
		duration: 100,
		ease: linear,
		onUpdate: (v: V) => output.push(typeof v === 'number' ? (Math.round(v) as V) : (v as V)),
		onComplete: () => {
...
describe('MainThreadAnimation', () => {
	test('Correctly performs an animation with default settings', async () => {
		return new Promise<void>((resolve) => testAnimate({ keyframes: [0, 100] }, [0, 20, 40, 60, 80, 100], resolve));
	});
			...
			animateValue({
				...
				onComplete: () => {
					expect(numeric).toEqual([
						0, 21, 68, 122, 170, 205, 225, 232, 231, 224, 215, 207, 200, 197, 195, 195, 196, 197, 199, 200,
						200, 201, 201, 201, 201, 200,
					]);
					resolve();
				},
			});
	...
	test('Correctly animates string spring', async () => {
		return new Promise<void>((resolve) => {
			...
			animateValue({
				...
				restSpeed: 10,
				restDelta: 0.5,
				onUpdate: (v) => string.push(Math.round(parseFloat(v as string))),
				onComplete: () => {
					expect(string).toEqual([0, 21, 68, 122, 170, 205, 225, 232, 231, 224, 215, 207, 200, 197, 195, 195, 196, 197, 199, 200]);
					resolve();
				},
			});
		});
	...
	test('Correctly animates from/to with a keyframes animation by default', async () => {
		return new Promise<void>((resolve) => {
			testAnimate({ keyframes: [50, 150] }, [50, 70, 90, 110, 130, 150], resolve);
		});
	});
	...
	test('Accepts delay', async () => {
		return new Promise<void>((resolve) => {
			testAnimate({ keyframes: [0, 100], delay: 100 }, [0, 0, 0, 0, 0, 0, 20, 40, 60, 80, 100], resolve);
		});
	});

	test('Accepts negative delay as elapsed', async () => {
		return new Promise<void>((resolve) => {
			testAnimate({ keyframes: [0, 100], delay: -50 }, [50, 70, 90, 100], resolve);
		});
	});

	test('Correctly repeats', async () => {
		return new Promise<void>((resolve) => {
			testAnimate({ keyframes: [0, 100], repeat: 1 }, [0, 20, 40, 60, 80, 100, 20, 40, 60, 80, 100], resolve);
		});
	});
	...
	test('Correctly applies repeatDelay', async () => {
		return new Promise<void>((resolve) => {
			testAnimate(
				...
				[0, 20, 40, 60, 80, 100, 100, 100, 100, 100, 100, 20, 40, 60, 80, 100, 100, 100, 100, 100, 100, 20, 40, 60, 80, 100],
				resolve
			);
		});
	});
	...
	test('Correctly applies repeatDelay to reverse', async () => {
		return new Promise<void>((resolve) => {
			testAnimate(
				...
				[0, 20, 40, 60, 80, 100, 100, 100, 100, 100, 100, 80, 60, 40, 20, 0, 0, 0, 0, 0, 0, 20, 40, 60, 80, 100],
				resolve
			);
		});
	});
	...
	test('Correctly applies repeatDelay to mirror', async () => {
		return new Promise<void>((resolve) => {
			testAnimate(
				...
				[0, 36, 64, 84, 96, 100, 100, 100, 100, 100, 100, 64, 36, 16, 4, 0, 0, 0, 0, 0, 0, 36, 64, 84, 96, 100],
				resolve
			);
		});
	});
	...
	test('Runs animations as an underdamped spring', async () => {
		return new Promise<void>((resolve) => {
			const output: number[] = [];
			const expected = [
				100, 371, 884, 1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005,
				1008, 1005, 1001, 998, 997, 998, 1000, 1001, 1001, 1001, 1000, 1000,
			];
			animateValue({
				keyframes: [100, 1000],
				type: 'spring',
	...
	test('Runs animations as an overdamped spring', async () => {
		return new Promise<void>((resolve) => {
			const output: number[] = [];
			const expected = [100, 571, 802, 909, 958, 981, 991, 996, 998, 999, 1000];
			animateValue({
				keyframes: [100, 1000],
				type: 'spring',
				stiffness: 300,
	...
	test('Runs animations as a critically damped spring', async () => {
		return new Promise<void>((resolve) => {
			const output: number[] = [];
			const expected = [
				100, 181, 338, 498, 635, 741, 821, 878, 918, 945, 964, 976, 984, 990, 993, 996, 997, 998, 999, 999, 1000,
			];
			animateValue({
				keyframes: [100, 1000],
				type: 'spring',
	...
	test('Repeats springs', async () => {
		return new Promise<void>((resolve) => {
			...
			const expected = [
				100, 371, 884, 1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005,
				1008, 1005, 1001, 998, 997, 998, 1000, 1001, 1001, 1001, 1000, 1000, 371, 884, 1259, 1343, 1204, 1006, 883,
				873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005, 1008, 1005, 1001, 998, 997, 998, 1000, 1001,
				1001, 1001, 1000, 1000,
			];
			animateValue({
				keyframes: [100, 1000],
	...
	test('Repeats springs with repeat delay', async () => {
		return new Promise<void>((resolve) => {
			...
			const expected = [
				100, 371, 884, 1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005,
				1008, 1005, 1001, 998, 997, 998, 1000, 1001, 1001, 1001, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000,
				371, 884, 1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005, 1008,
				1005, 1001, 998, 997, 998, 1000, 1001, 1001, 1001, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 1000, 371, 884,
				1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005, 1008, 1005, 1001,
	...
	test("Repeats springs as 'reverse'", async () => {
		return new Promise<void>((resolve) => {
			...
			const expected = [
				100, 371, 884, 1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005,
				1008, 1005, 1001, 998, 997, 998, 1000, 1001, 1001, 1001, 1000, 1000, 1000, 1001, 1001, 1001, 1000, 998, 997,
				998, 1001, 1005, 1008, 1005, 996, 984, 980, 991, 1018, 1046, 1050, 1011, 937, 873, 883, 1006, 1204, 1343,
				1259, 884, 371, 100,
			];
			animateValue({
				keyframes: [100, 1000],
	...
	test("Repeats springs as 'reverse' with repeatDelay", async () => {
		return new Promise<void>((resolve) => {
			const output: number[] = [];
			const expected = [
				100, 884, 1343, 1006, 873, 1011, 1046, 991, 984, 1005, 1005, 998, 998, 1001, 1001, 1000, 1000, 1000, 1000,
				1001, 1001, 998, 998, 1005, 1005, 984, 991, 1046, 1011, 873, 1006, 1343, 884, 100, 100, 100, 100, 884, 1343,
				1006, 873, 1011, 1046, 991, 984, 1005, 1005, 998, 998, 1001, 1001, 1000,
			];
			animateValue({
				keyframes: [100, 1000],
				stiffness: 300,
	...
	test("Repeats springs as 'mirror'", async () => {
		return new Promise<void>((resolve) => {
			...
			const expected = [
				100, 371, 884, 1259, 1343, 1204, 1006, 883, 873, 937, 1011, 1050, 1046, 1018, 991, 980, 984, 996, 1005,
				1008, 1005, 1001, 998, 997, 998, 1000, 1001, 1001, 1001, 1000, 1000, 729, 216, -159, -243, -104, 94, 217,
				227, 163, 89, 50, 54, 82, 109, 120, 116, 104, 95, 92, 95, 99, 102, 103, 102, 100, 99, 99, 99, 100, 100,
			];
			animateValue({
				keyframes: [100, 1000],
				type: 'spring',
	...
	test("Repeats springs as 'mirror' with repeatDelay", async () => {
		return new Promise<void>((resolve) => {
			...
			const expected = [
				100, 884, 1343, 1006, 873, 1011, 1046, 991, 984, 1005, 1005, 998, 998, 1001, 1001, 1000, 1000, 1000, 1000,
				216, -243, 94, 227, 89, 54, 109, 116, 95, 95, 102, 102, 99, 99, 100, 100, 100, 100, 884, 1343, 1006, 873,
				1011, 1046, 991, 984, 1005, 1005, 998, 998, 1001, 1001, 1000,
			];
			animateValue({
				keyframes: [100, 1000],
				type: 'spring',
	...
	test('Runs animations as a decay with modifyTarget', async () => {
		return new Promise<void>((resolve) => {
			const output: number[] = [];
			const expected = [100, 213, 277, 313, 334, 345, 352, 355, 357, 358, 359, 360];
