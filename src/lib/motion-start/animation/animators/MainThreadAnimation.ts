...
export class MainThreadAnimation<T extends string | number> extends BaseAnimation<T, ResolvedData<T>> {
	...
	protected initPlayback(keyframes: ResolvedKeyframes<T>) {
		...
		const generatorFactory = isGenerator(type)
			? type
			: (type !== false ? generators[type] : undefined) || keyframesGeneratorFactory;
...
// Legacy interface
export function animateValue(options: ValueAnimationOptionsWithRenderContext<any>): MainThreadAnimation<any> {
