...
function getElementAnimationState(element: Element) {
	const animationState = state.get(element) || new Map<string, NativeAnimation>();
...
export class NativeAnimation implements AnimationPlaybackControls {
	...
	constructor(
		...
		if (!supportsWaapi()) {
			...
			this.animation = startWaapiAnimation(element, valueName, valueKeyframes as string[], options);
	...
	get time() {
		if (this.animation) {
			return millisecondsToSeconds((this.animation?.currentTime as number) || 0);
		}
		return 0;
	}
