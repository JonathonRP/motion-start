...
import { appearStoreId } from './store-id.svelte';
import { startWaapiAnimation } from '../animators/waapi/index.svelte';
import type { NativeAnimationOptions } from '../animators/waapi/types';
import { optimizedAppearDataId } from './data-id.svelte';
import { handoffOptimizedAppearAnimation } from './handoff.svelte';
import { appearAnimationStore, type AppearStoreEntry, appearComplete } from './store.svelte';
import { noop } from '../../utils/noop';
import './types';
import { getOptimisedAppearId } from './get-appear-id.svelte';
...
export function startOptimizedAppearAnimation(
	...
	if (!readyAnimation) {
		...
		window.MotionCheckAppearSync = (visualElement: WithAppearProps, valueName: string, value: MotionValue) => {
			...
			const removeSyncCheck = value.on('change', (latestValue: string | number) => {
