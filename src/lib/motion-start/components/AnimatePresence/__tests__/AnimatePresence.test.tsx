 ...
 * Original path: packages/framer-motion/src/components/AnimatePresence/__tests__/AnimatePresence.test.tsx
 *
...
describe("AnimatePresence with custom components", () => {
    ...
    test("Exit variants are triggered with `AnimatePresence.custom` throughout the tree", async () => {
        ...
        expect(xParent.get()).toBe(200)
        expect(xChild.get()).toBe(200)
    })
})

test("Sibling AnimatePresence wrapped in LayoutGroup", async () => {
    const promise = new Promise<number>((resolve) => {
        const Component = ({ isVisible }: { isVisible: boolean }) => {
            return (
                <LayoutGroup>
