...
const updateChildLookup = (children: { key: number }[], allChild: Map<string | number, { key: number }>) => {
...
$effect(() => {
	...
	if (isInitialRender) {
		...
		untrack(() => {
			...
			const presentKeys = untrack(() => renderedChildren.filter((c) => c.present).map(getChildKey));

			// Mark children that are exiting (were present but not in target)
			const numPresent = presentKeys.length;
			for (let i = 0; i < numPresent; i++) {
				const key = presentKeys[i] as string | number;
				if (targetKeys.indexOf(key as any) === -1 && !exiting.has(key)) {
					exiting.add(key);
				}
			}
				...
				exiting.forEach((key) => {
					...
					const onExit = () => {
						untrack(() => {
							...
							const removeIndex = renderedChildren.findIndex((c) => getChildKey(c) === key);
							if (removeIndex >= 0) {
