...
export function createAnimationState<I>(visualElement: VisualElement<I>): AnimationState {
	...
	function animateChanges(changedActiveType?: AnimationType) {
		...
		const context = getVariantContext(visualElement.parent ?? undefined) || {};
		...
		for (let i = 0; i < numAnimationTypes; i++) {
			...
			const prop = props[type] !== undefined ? props[type] : context[type as keyof typeof context];
			 ...
			 */
			let isInherited = prop === context[type as keyof typeof context] && prop !== props[type] && propIsVariant;
