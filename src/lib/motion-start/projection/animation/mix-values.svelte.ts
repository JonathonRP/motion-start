...
const asNumber = (value: string | number) => (typeof value === 'string' ? Number.parseFloat(value) : value);

const isPx = (value: string | number) => typeof value === 'number' || px.test(value);
...
export function mixValues(
	...
	if (follow.rotate || lead.rotate) {
		target.rotate = mixNumber((follow.rotate as number) || 0, (lead.rotate as number) || 0, progress);
	}
}
...
function compress(min: number, max: number, easing: EasingFunction): EasingFunction {
	return (p: number) => {
