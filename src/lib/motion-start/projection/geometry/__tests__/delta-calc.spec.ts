...
describe('calcAxisDelta', () => {
	test('correctly calculates a delta that, when applied to source, will make it the same as target', () => {
		...
		applyAxisDelta(source, delta.x.translate, delta.x.scale, delta.x.originPoint);
...
describe('calcRelativeBox', () => {
	test('correctly calculates relative box position', () => {
...
describe('calcRelativePosition', () => {
	test('correctly calculates relative position from layout and parent', () => {
		const target = createBox();
	...
	test('handles identical layout and parent boxes', () => {
		const target = createBox();
