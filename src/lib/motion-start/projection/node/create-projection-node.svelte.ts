				...
				this.addEventListener(
					...
					({ delta, hasLayoutChanged, hasRelativeTargetChanged, layout: newLayout }: LayoutUpdateData) => {
						...
						const layoutTransition =
							this.options.transition || visualElement.getDefaultTransition() || defaultLayoutTransition;
						 ...
						 */
						const targetChanged =
							!this.targetLayout || !boxEqualsRounded(this.targetLayout, newLayout) || hasRelativeTargetChanged;
		...
		registerSharedNode(layoutId: string, node: IProjectionNode<I>) {
			...
			node.promote({
				...
				preserveFollowOpacity:
					config && config.shouldPreserveFollowOpacity ? config.shouldPreserveFollowOpacity(node) : undefined,
			});
		}
...
function shouldAnimatePositionOnly(animationType: string | undefined, snapshot: Box, layout: Box) {
	return (
		animationType === 'position' ||
		(animationType === 'preserve-aspect' && !isNear(aspectRatio(snapshot), aspectRatio(layout), 0.2))
	);
}
