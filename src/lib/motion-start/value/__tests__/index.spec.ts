/**
 * Ported from framer-motion v11.11.11
...
describe('MotionValue velocity calculations', () => {
	...
	test("Velocity is capped to the last estimated frame when value hasn't been updated in a long time, and is then updated outside frameloop", async () => {
		...
		value.set(1);
		frameData.isProcessing = false;
		// Simulate time passing: await lets the queueMicrotask(clearTime) in time.set() run,
		// so next time.now() reads frameData.timestamp (1000) instead of cached value (10)
