...
import { hex } from '../color/hex.js';
import { rgba, rgbUnit } from '../color/rgba.js';
import { hsla } from '../color/hsla.js';
import { color } from '../color/index.js';
import { degrees, percent, progressPercentage, px } from '../numbers/units.js';
...
describe('regex', () => {
	it('should correctly identify values', () => {
		...
		expect(singleColorRegex.test('rgba(161 0 246 / 0)')).toBe(true);
		expect(singleColorRegex.test('rgba(161 0 246/0)')).toBe(true);
		expect(singleColorRegex.test('rgba(161 0 246 / 0) rgba(161 0 246 / 0)')).toBe(false);
		...
		expect('rgba(161 0 246 / 0) rgba(161 0 246 / 0)'.match(colorRegex)?.length).toEqual(2);

		expect(colorRegex.test('rgba(161 0 246/0) rgba(161 0 246/0)')).toBe(true);

		expect('rgba(161 0 246/0) rgba(161 0 246/0)'.match(colorRegex)?.length).toEqual(2);

		expect('a0.9b 1 2 -3 -4.0 -.5 -0.6 a.7b 800 10.0009'.match(floatRegex)).toEqual([
			'0.9',
			'1',
			'2',
			'-3',
		...
		expect('hsla(177, 37.4978%, 76.66804%, 1)'.match(floatRegex)).toEqual(['177', '37.4978', '76.66804', '1']);
	});
});
...
describe('complex value type', () => {
	...
	it('parse converts string to array', () => {
		...
		expect(complex.parse(MIXED)).toEqual([0, 0, 0, { red: 161, green: 0, blue: 246, alpha: 0 }]);
		...
		expect(complex.parse('0px 0px 0px #F00')).toEqual([0, 0, 0, { red: 255, green: 0, blue: 0, alpha: 1 }]);
		expect(complex.parse('0px 0px 0px #F000')).toEqual([0, 0, 0, { red: 255, green: 0, blue: 0, alpha: 0 }]);
		expect(complex.parse('0px 0px 0px #00FF0000')).toEqual([0, 0, 0, { red: 0, green: 255, blue: 0, alpha: 0 }]);
	});
	...
	it('createTransformer returns a transformer function that correctly inserts values', () => {
		...
		const transformMixedExpo = complex.createTransformer('0px 0px 0px rgba(161, 0, 246, 0)');
		expect(
			transformMixedExpo([
				0,
	...
	it('can create an animatable "none"', () => {
		expect(complex.getAnimatableNone('100% 0px var(--grey, 100) #fff')).toBe(
...
describe('combination values', () => {
	it('should test correctly', () => {
		...
		expect(complex.test('20px 20px 10px inset rgba(255, 255, 255, 1), 20px')).toEqual(true);
		expect(complex.test('linear-gradient(0.25turn, #3f87a6, #ebf8e1, #f69d3c')).toEqual(true);
	});

	it('should parse into an array', () => {
		expect(complex.parse('0px 10px #fff')).toEqual([0, 10, { red: 255, green: 255, blue: 255, alpha: 1 }]);
		...
		expect(complex.parse('20px 20px 10px inset rgba(255, 255, 255, 1)')).toEqual([
			20,
			20,
			10,
			{ red: 255, green: 255, blue: 255, alpha: 1 },
		]);
		expect(complex.parse('20px 20px 10px inset #fff, 20px 20px 10px inset rgba(255, 255, 255, 1)')).toEqual([
			20,
			20,
			10,
			{ red: 255, green: 255, blue: 255, alpha: 1 },
		...
		expect(complex.parse('linear-gradient(1deg, rgba(255, 255, 255, 1))')).toEqual([
			1,
			{
				red: 255,
				green: 255,
		...
		expect(complex.parse('linear-gradient(217deg, rgba(255,0,0,.8), rgba(255,0,0,0) 70.71%)')).toEqual([
			217,
			{
				red: 255,
				green: 0,
		...
		expect(complex.parse('radial-gradient(circle at 50% 25%, #e66465, #9198e5)')).toEqual([
			50,
			25,
	...
	it('should create a transformer', () => {
		const animatable = complex.parse('20px 20px 10px inset rgba(255, 255, 255, 1), 20px 20px 10px inset #fff');
		...
		const gradient = complex.parse('linear-gradient(217deg, rgba(255,0,0,.8), rgba(255,0,0,0) 70.71%)');
