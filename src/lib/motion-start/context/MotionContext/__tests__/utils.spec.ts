...
describe('getCurrentTreeVariants', () => {
	test('It returns the correct variant to render currently', () => {
		...
		expect(getCurrentTreeVariants({ initial: false, animate: { opacity: 0 } }, { animate: 'hello' })).toEqual({
			animate: 'hello',
		});

		expect(getCurrentTreeVariants({ initial: false, animate: 'hello' }, {})).toEqual({
			initial: false,
			animate: 'hello',
		});
		...
		expect(getCurrentTreeVariants({ initial: false, animate: ['a', 'b'] }, { initial: 'c', animate: 'b' })).toEqual({
			initial: false,
			animate: ['a', 'b'],
		});
		...
		expect(
			getCurrentTreeVariants({ initial: false, animate: animationControls() }, { initial: 'a', animate: 'b' })
